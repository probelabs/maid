...
22. [Label Quote In Unquoted](#22-label-quote-in-unquoted)
...
| 22 | [label quote in unquoted](#22-label-quote-in-unquoted) | INVALID | INVALID | ✅ safe |
...
## 22. Label Quote In Unquoted

📄 **Source**: [`label-quote-in-unquoted.mmd`](./invalid/label-quote-in-unquoted.mmd)
...
```mermaid
graph TD
    A[User Request: "Amend docs"] --> B{route-projects skill};
    B --|Before PR|--> C["'tyk-docs' not in projects list"];
    C --> D[Empty Result];
    D --> G[Engineer Fails];
    B --|After PR|--> E["'tyk-docs' is in projects list"];
    E --> F["Project 'tyk-docs' Selected"];
    F --> H[Engineer Proceeds];
...
```
error[FL-LABEL-QUOTE-IN-UNQUOTED]: Quotes are not allowed inside unquoted node labels. Use &quot; for quotes or wrap the entire label in quotes.
at test-fixtures/flowchart/invalid/label-quote-in-unquoted.mmd:2:21
  1 | graph TD
  2 |     A[User Request: "Amend docs"] --> B{route-projects skill};
    |                     ^^^^^^^^^^^^
  3 |     B --|Before PR|--> C["'tyk-docs' not in projects list"];
hint: Example: I[Log &quot;processing N items&quot;] or I["Log \"processing N items\""]
```
